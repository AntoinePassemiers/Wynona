...
def configuration(parent_package=str(), top_path=None):
    ...
    config.set_options(ignore_setup_xxx_py=True,
                       ...
                       quiet=True)
    config.add_subpackage('wynona')
    return config

setup_args = {
    'name' : 'wynona',
    'version' : '1.0.0',
    'description' : 'Protein Contact Prediction based on a Fully-convolutional Neural Architecture',
    'author' : 'Antoine Passemiers',
...
for source_file in source_files:
    ...
    extension_name = '.'.join(['wynona', source_file.replace('/', '.').replace('\\', '.')])
